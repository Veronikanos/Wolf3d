...
/*   Updated: 2019/07/26 21:13:00 by vtlostiu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	pix->line_height = (Uint32)(HEIGHT / wall_dist);
	pix->from_to.x = -(pix->line_height >> 1u) + HEIGHT_H;
	pix->from_to.y = (pix->line_height >> 1u) + HEIGHT_H;
	if (pix->from_to.x < 0)
		pix->from_to.x = 0;
	if (pix->from_to.y >= (int)HEIGHT)
...
void			game_process(t_pix *pix)
{
	...
	while (++i < WIDTH)
	{
		...
		pix->ray_map_cord = (t_xy){ (int)pix->player.pos.x,
